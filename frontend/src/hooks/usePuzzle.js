...
export function usePuzzle(onSolved) {
  ...
  useEffect(()=>{if(isRunning&&!isSolved){ref.current=setInterval(()=>setTimer(t=>t+1),1000);}return()=>clearInterval(ref.current);},[isRunning,isSolved]);
  const reset=useCallback(async()=>{clearInterval(ref.current);setGrid(Array.from({length:5},()=>Array(5).fill("")));setSelectedIdx(null);setTimer(0);setIsRunning(false);setIsSolved(false);setHintPenalty(0);setErrorCells([]);setMessage("Select a letter from the pool, then click a cell.");const letters=await fetchPool();setPool(letters.map(l=>({letter:l,used:false})));},[]);
  ...
  const check=useCallback(async()=>{if(isSolved)return null;setErrorCells([]);if(!grid.every(r=>r.every(c=>c!==""))){setMessage("Not all cells filled yet.");return"incomplete";}const result=await validateGrid(grid);if(result.is_valid){setIsSolved(true);setIsRunning(false);const total=timer+hintPenalty;await saveCompletion(total,hintPenalty);setMessage("Solved in "+formatTime(total)+"!");if(onSolved)onSolved(total);return"success";}setErrorCells(result.errors.map(e=>e.row*5+e.col));setMessage(result.errors.length+" letters still incorrect.");return"errors";},[isSolved,grid,timer,hintPenalty,onSolved]);
  const hint=useCallback(()=>{if(isSolved)return null;if(!isRunning)setIsRunning(true);setErrorCells([]);for(let r=0;r<5;r++)for(let c=0;c<5;c++)if(grid[r][c]!==SATOR_SQUARE[r][c]){const correct=SATOR_SQUARE[r][c];setGrid(g=>{const ng=g.map(row=>[...row]);if(ng[r][c]!==""){const wrong=ng[r][c];ng[r][c]="";setPool(p=>p.map(i=>i.used&&i.letter===wrong?{...i,used:false}:i));}ng[r][c]=correct;return ng;});setPool(p=>p.map(i=>!i.used&&i.letter===correct?{...i,used:true}:i));setSelectedIdx(null);setHintPenalty(h=>h+15);setMessage("Hint: \""+correct+"\" at row "+(r+1)+", col "+(c+1)+". +15s");return{row:r,col:c};}return null;},[isSolved,isRunning,grid]);
